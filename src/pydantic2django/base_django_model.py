...
class Pydantic2DjangoStorePydanticObject(models.Model):
    """
...
class Pydantic2DjangoBaseClass(models.Model):
    ...
    # Custom configuration class for Pydantic-related settings
    ...
    def update_fields_from_pydantic(self, pydantic_obj: Any) -> None:
        """
    ...
    def _get_module_path(self) -> str:
        ...
        Returns:
            The module path as a string
        """
        # Try to get the module path from the PydanticConfig class
        cls = self.__class__
        module_path = getattr(cls.PydanticConfig, "module_path", None)

        if not module_path:
            raise ValueError(
                f"No module path found for {self.object_type}. "
